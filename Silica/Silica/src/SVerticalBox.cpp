...
namespace Silica {
	...
	void SVerticalBox::arrangeChildren(const Geometry& allocatedGeometry) {
		...
		for (const Slot& slot : m_slots) {
			...
			if (!isFirstChild) currentY += m_spacing;
			isFirstChild = false;
			...
			childGeo.size.x = allocatedGeometry.size.x - (slot.padding.x * 2.0f);
	...
	EventReply SVerticalBox::onMouseButtonDown(const Geometry& allocatedGeometry, const Vec2& mousePos) {
		for (const Slot& slot : m_slots) {
	...
	EventReply SVerticalBox::onMouseButtonUp(const Geometry& allocatedGeometry, const Vec2& mousePos) {
		for (const Slot& slot : m_slots) {
			if (slot.child && slot.child->getAllocatedGeometry().contains(mousePos)) {
